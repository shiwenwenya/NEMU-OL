...
void syscallOpen(struct StackFrame *sf)
{
	...
	{ // try to create file
		...
		if ((sf->edx >> 3) % 2 == 0)
		{
			...
			char father_path[NAME_LENGTH];
			for (int i = 0; i < size + 1; i++)
				father_path[i] = *(str + i);
			father_path[size + 1] = '\0';
			ret = readInode(&sBlock, gDesc, &fatherInode, &fatherInodeOffset, father_path);
			if (ret == -1)
			{
				pcb[current].regs.eax = -1;
				return;
...
void syscallWrite(struct StackFrame *sf)
{
	...
	if (sf->ecx < MAX_DEV_NUM || sf->ecx >= MAX_FILE_NUM + MAX_DEV_NUM || file[sf->ecx - MAX_DEV_NUM].state == 0)
	{
		pcb[current].regs.eax = -1;
		return;
	}
...
void syscallWriteFile(struct StackFrame *sf)
{
	...
	int num = quotient;
	int count = 0;
	int i = sBlock.blockSize - remainder < size ? sBlock.blockSize - remainder : size;

	while (count < size)
	{
		readBlock(&sBlock, &inode, num, buffer);
		if (count == 0)
			MemCpy(str, buffer + remainder, i);
		else
		{
			i = size - count < sBlock.blockSize ? size - count : sBlock.blockSize;
			MemCpy(str + count, buffer, i);
		}
		count += i;
		writeBlock(&sBlock, &inode, num, buffer);
		num++;
	}
...
void syscallReadFile(struct StackFrame *sf)
{
	...
	while (count < size)
	{
		...
		num++;
		if (count == 0)
			MemCpy(buffer + remainder, str, i);
		else
		{
			i = size - count < sBlock.blockSize ? size - count : sBlock.blockSize;
			MemCpy(buffer, str + count, i);
		}
		count += i;
	}
...
void syscallLseek(struct StackFrame *sf)
{
	...
	if (FCBindex < 0 || FCBindex >= MAX_FILE_NUM || file[FCBindex].state == 0)
	{ // out of range
		pcb[current].regs.eax = -1;
		return;
	}
